...
contract TestModuleManager_InstallModule is TestModuleManagement_Base {
    ...
    function test_RevertIf_ModuleAlreadyInstalled() public {
        ...
        bytes memory expectedRevertReason = abi.encodeWithSignature("LinkedList_EntryAlreadyInList(address)", address(mockValidator));
    ...
    function test_InstallModule_MultiTypeInstall() public {
        ...
        bytes memory fallbackInstallData =
            abi.encodePacked(bytes4(GENERIC_FALLBACK_SELECTOR), CALLTYPE_SINGLE, bytes1(uint8(MODULE_TYPE_FALLBACK)), fallbackConfig);
        ...
        bytes memory callData = abi.encodeWithSelector(IModuleManager.installModule.selector, MODULE_TYPE_MULTI, address(mockMulti), multiInstallData);

        installModule(callData, MODULE_TYPE_MULTI, address(mockMulti), EXECTYPE_DEFAULT);

        assertTrue(BOB_ACCOUNT.isModuleInstalled(MODULE_TYPE_VALIDATOR, address(mockMulti), ""), "Module should be installed as validator");
        assertEq(mockMulti.getConfig(address(BOB_ACCOUNT), MODULE_TYPE_VALIDATOR), validatorConfig, "Module should be properly configured as validator");

        assertTrue(BOB_ACCOUNT.isModuleInstalled(MODULE_TYPE_EXECUTOR, address(mockMulti), ""), "Module should be installed as executor");
        assertEq(mockMulti.getConfig(address(BOB_ACCOUNT), MODULE_TYPE_EXECUTOR), executorConfig, "Module should be properly configured as executor");
        ...
        assertEq(mockMulti.getConfig(address(BOB_ACCOUNT), MODULE_TYPE_FALLBACK), fallbackConfig, "Module should be properly configured as fallback");

        assertTrue(BOB_ACCOUNT.isModuleInstalled(MODULE_TYPE_HOOK, address(mockMulti), ""), "Module should be installed as hook");
        assertEq(mockMulti.getConfig(address(BOB_ACCOUNT), MODULE_TYPE_HOOK), hookConfig, "Module should be properly configured as hook");
    }
    ...
    function test_InstallFallbackHandler_WithCustomData() public {
        ...
        assertFalse(BOB_ACCOUNT.isModuleInstalled(MODULE_TYPE_FALLBACK, address(mockHandler), customData), "FallbackHandler should not be installed initially");
        ...
        assertTrue(
            BOB_ACCOUNT.isModuleInstalled(MODULE_TYPE_FALLBACK, address(mockHandler), customData), "FallbackHandler with custom data should be installed"
        );
    }
    ...
    function test_RevertIf_ReinstallFallbackHandler() public {
        ...
        // First install
        bytes memory callDataFirstInstall =
        ...
        bytes memory callDataReinstall = abi.encodeWithSelector(IModuleManager.installModule.selector, MODULE_TYPE_FALLBACK, address(mockHandler), customData);
        ...
        PackedUserOperation[] memory userOps = buildPackedUserOperation(BOB, BOB_ACCOUNT, EXECTYPE_DEFAULT, executionReinstall, address(VALIDATOR_MODULE), 0);

        bytes memory expectedRevertReason = abi.encodeWithSignature("FallbackAlreadyInstalledForSelector(bytes4)", bytes4(GENERIC_FALLBACK_SELECTOR));
