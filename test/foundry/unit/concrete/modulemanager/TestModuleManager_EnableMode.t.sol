...
contract TestModuleManager_EnableMode is Test, TestModuleManagement_Base {
    ...
    function test_EnableMode_Success_No7739() public {
        ...
        bytes memory enableModeSigPrefix = abi.encodePacked(
            moduleToEnable, MODULE_TYPE_MULTI, bytes4(uint32(multiInstallData.length)), multiInstallData, bytes4(uint32(enableModeSig.length)), enableModeSig
        );
    ...
    function test_EnableMode_Success_7739_Nested_712() public {
        ...
        bytes memory enableModeSig = abi.encodePacked(t.r, t.s, t.v, appDomainSeparator, structHash, contentsType, uint16(contentsType.length)); //prepare 7739
            // sig
    ...
    function test_EnableMode_Success_DeployAccount() public {
        ...
        //make calldata
        userOp.callData = prepareERC7579SingleExecuteCallData(EXECTYPE_DEFAULT, address(counter), 0, abi.encodeWithSelector(Counter.incrementNumber.selector));
        ...
        assertTrue(EVE_ACCOUNT.isModuleInstalled(MODULE_TYPE_VALIDATOR, address(mockMultiModule), ""), "Module should be installed as validator");
        assertTrue(EVE_ACCOUNT.isModuleInstalled(MODULE_TYPE_EXECUTOR, address(mockMultiModule), ""), "Module should be installed as executor");
    }
    ...
    function test_EnableMode_FailsWithWrongValidationModuleInEnableModeSig() public {
        ...
        bytes memory expectedRevertReason =
            abi.encodeWithSelector(FailedOpWithRevert.selector, 0, "AA23 reverted", abi.encodeWithSelector(ValidatorNotInstalled.selector, invalidValidator));
    ...
    function test_EnableMode_FailsWithWrongSig() public {
        ...
        bytes memory expectedRevertReason =
            abi.encodeWithSelector(FailedOpWithRevert.selector, 0, "AA23 reverted", abi.encodeWithSelector(EnableModeSigError.selector));
    ...
    function test_EnableMode_FailsWhenModuleAlreadyInstalled() public {
        ...
        bytes memory revertReason = abi.encodeWithSignature("LinkedList_EntryAlreadyInList(address)", address(mockMultiModule));

        op.signature = abi.encodePacked(enableModeSigPrefix, op.signature);
        PackedUserOperation[] memory userOps = new PackedUserOperation[](1);
        userOps[0] = op;

        bytes memory expectedRevertReason = abi.encodeWithSelector(FailedOpWithRevert.selector, 0, "AA23 reverted", revertReason);
    ...
    function test_EnableMode_FailsWithWrongModuleType() public {
        ...
        (bytes memory multiInstallData, bytes32 hashToSign,) = makeInstallDataAndHash(address(BOB_ACCOUNT), MODULE_TYPE_EXECUTOR, userOpHash); // Use EXECUTOR
            // type instead of MULTI
        ...
        bytes memory expectedRevertReason = abi.encodeWithSelector(
            FailedOpWithRevert.selector, 0, "AA23 reverted", abi.encodeWithSelector(ValidatorNotInstalled.selector, address(moduleToEnable))
        );
    ...
    function makeDraftOp(address moduleToEnable) internal view returns (PackedUserOperation memory op) {
        ...
        op.callData = prepareERC7579SingleExecuteCallData(EXECTYPE_DEFAULT, address(counter), 0, abi.encodeWithSelector(Counter.incrementNumber.selector));
    }
    ...
    function makeInstallDataAndHash(
        ...
        internal
        view
        returns (bytes memory multiInstallData, bytes32 eip712Hash, bytes32 structHash)
    {
        ...
        structHash = keccak256(abi.encode(MODULE_ENABLE_MODE_TYPE_HASH, address(mockMultiModule), moduleType, userOpHash, keccak256(multiInstallData)));
